<?php include_once 'inc/header.php' ?>
<div class="svis svis_bg1">
	<div class="con_width">
	    <span class="tit"><span>STOMPlus2</span></span>
	    <span class="txt">매출_매출자료생성</span>  
	</div>
<article>
	<div id="_snb">
		<ul class="con_width">
			<li><a href="/"><i class="material-icons">home</i></a></li>
			<li class="_ovv">
				<a href="INV.php"><span>매출</span></a>
				<ul>
					<li><a href="TAKEIN_1.php">매입</a></li> 
					<li><a href="JAEGO_1.php">재고</a></li> 
                    <li><a href="CALCULATE_1.php">정산</a></li>
                    <li><a href="LIST_1.php">현황2</a></li>
                    <li><a href="CODE_1.php">코드관리</a></li>
                    <li><a href="FUNCTION_1.php">부가기능</a></li>
				</ul>
			</li>

			<li class="_ov">
				<a href="INV_4.php"><span><em>매출자료생성</em></span></a>
				<ul>
					<li><a href="INV_5.php">품목코드대체</a></li>
					<li><a href="INV_1.php">견적관리</a></li>
					<li><a href="INV_2.php">수주관리</a></li>
					<li><a href="INV_3.php">매출관리</a></li>
				</ul> 
			</li>
		</ul>
	</div>
	<div class="sub_wrap">
		<img src="images/inv_2.png">
		<p>
			<span>▶ 일일 매출자료 생성</span> : 작성된 수주서 들을 일괄적으로 매출관리 상의 자료로 생성 시에 사용
		</p>
	</div>
</article>
<?php include_once 'inc/footer.php' ?>
</div>